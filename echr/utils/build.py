...
def load_workflow(console, args):
    ...
    if os.path.isdir('./workflows'):
        available_workflows = [f.split('.')[0] for f in os.listdir('./workflows') if
        ...
        if workflow not in available_workflows:
            ...
            try:
                with open(os.path.join('./workflows', '{}.yml'.format(workflow))) as f:
    ...
    all_replaced = True
    try:
        for i, step in enumerate(workflow_steps.get('steps', [])):
            for arg in step.get('args', {}):
                val, rc = parse_argument(str(step['args'][arg]), cli_args=args)
                if rc != 0:
                    workflow_steps['steps'][i]['args'][arg] = val
                    if rc < 0:
